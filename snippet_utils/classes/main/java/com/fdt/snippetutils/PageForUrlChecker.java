...
@Configuration
@EnableAutoConfiguration(exclude={AnchorTitleReplacer.class,AnchorTitleScrapper.class, KeywordShortUrlInserter.class, PageForUrlChecker.class})
...
public class PageForUrlChecker {
	...
	public static void main(String[] args) {
		...
		try{
			ApplicationContext ctx = SpringApplication.run(PageForUrlChecker.class, args);
	...
	@PostConstruct
	private void init(){
		this.pageListFilePath = cfgManager.getProperty(SITE_LIST_FILE_PATH_LABEL);

		this.maxThreadCount = Integer.valueOf(cfgManager.getProperty(MAX_THREAD_COUNT_LABEL));

		this.isLinkWrapped = Boolean.valueOf(cfgManager.getProperty(IS_LINK_WRAPPED_LABEL));

		this.stringForCheck = cfgManager.getProperty(STRING_FOR_CHECK_LABEL);
	}
